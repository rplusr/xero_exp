...
// behaviour.
import { DIRS, DIR_COUNT, turnDelta } from './geometry.js';
import { chance, intRange, pick } from './rng.js';
const SEG_LEN = [2, 6];
function gridStep(d, cells) {
    const raw = DIRS[d];
    return { x: raw.dx * cells, y: raw.dy * cells };
}
export function verticesOf(start, segments) {
    const verts = [start];
    let cur = start;
...
function angleToDirIndex(angle) {
    let best = 0;
 ...
 */
function walk(rng, startDir, steps) {
    const segs = [];
    let curDir = startDir;
    const recentDeltas = [];
    for (let i = 0; i < steps; i++) {
        ...
        if (recentDeltas.length > 6)
            recentDeltas.shift();
        curDir = chosen;
    }
    return segs;
}
function trimReversal(segs, junctionDir, fromStart) {
    let out = segs;
    while (out.length) {
...
export function generateComposition(rng, opts) {
    const lines = [];
    ...
    for (let i = 0; i < opts.lineCount; i++) {
        ...
        if (canBundle) {
            const source = trunkPoolSize > 0 && chance(rng, 0.8)
            ...
            if (srcVerts.length >= 3) {
                ...
                const cloneSegs = source.segments
                ...
                const joins = new Set([leadIn.length, leadIn.length + cloneSegs.length]);
