...
import { generateComposition } from './pathgen.js';
import { hashSeed, mulberry32, randomSeedString } from './rng.js';
import { renderComposition } from './svg.js';
import { createDrift, runDrawIn } from './animate.js';
...
function densityToCellSize(density) {
    const t = (density - 1) / 9;
    return lerp(72, 22, Math.max(0, Math.min(1, t)));
}
export class TravelLinesApp {
    constructor(svg, wrapper) {
        this.svg = svg;
        this.wrapper = wrapper;
        this.config = defaultConfig();
        this.generated = [];
        this.driftRef = { current: null };
        this.drift = createDrift(this.driftRef);
