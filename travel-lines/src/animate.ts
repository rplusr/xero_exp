...
import { RenderHandles } from './svg.js';

export function runDrawIn(handles: RenderHandles, enabled: boolean, reducedMotion: boolean): void {
...
export interface DriftHandleRef {
  current: RenderHandles | null;
}
...
export function createDrift(ref: DriftHandleRef): DriftController {
  let rafId = 0;
  let running = false;
