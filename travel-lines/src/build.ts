...
import { GeneratedLine, verticesOf } from './pathgen.js';
...
export interface RenderStation {
  p: Point;
  /** Approximate fraction (0..1) along the line's draw-in animation. */
  t: number;
}
...
function pixelVerticesOf(line: GeneratedLine, cellSize: number, spacing: number): Point[] {
...
export function buildComposition(lines: GeneratedLine[], opts: BuildOptions): RenderComposition {
  ...
  const allLinePieceEndpoints: { lineId: number; p0: Point; p1: Point }[] = [];
  ...
  for (const line of lines) {
    ...
    if (opts.stations) {
      ...
      for (const idx of line.joins) {
        if (idx > 0 && idx < lastIdx) stations.push({ p: pixelVerts[idx], t: idx / lastIdx });
      }
    }
  ...
  if (opts.stations) {
    ...
    for (const c of crossings) {
      if (!deduped.some((d) => Math.hypot(d.x - c.x, d.y - c.y) < 10)) deduped.push(c);
    }
